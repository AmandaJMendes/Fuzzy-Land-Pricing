...
area['small']      = fuzz.trimf(area.universe, [100, 100, 300])
area['average']    = fuzz.trimf(area.universe, [200, 400, 700])
area['large']      = fuzz.trimf(area.universe, [600, 1000, 1400])
area['very large'] = fuzz.trimf(area.universe, [1000, 2000, 2000])
# area.view()
...
rules.append(ctrl.Rule(area['small'] & dist_to_av['close'], price['average']))
rules.append(ctrl.Rule(area['average'] & dist_to_av['average'] & dist_to_beach['average'], price['average']))
rules.append(ctrl.Rule(area['large'] & dist_to_beach['close'], price['average']))

rules.append(ctrl.Rule(area['average'] & (dist_to_av['close'] & dist_to_beach['close']), price['high']))
rules.append(ctrl.Rule(area['large'] & dist_to_av['close'], price['high']))
